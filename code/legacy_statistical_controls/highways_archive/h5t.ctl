...
compute ial=lnauto*lnplabor.
compute iau=lnauto*lnpsurf.
compute iai=lnauto*lnirate.
compute iam=lnauto*lnmiles.

compute icc=lncomb*lncomb.
compute ics=lncomb*lnsing.
compute icl=lncomb*lnplabor.
compute icu=lncomb*lnpsurf.
compute ici=lncomb*lnirate.
compute icm=lncomb*lnmiles.

compute iss=lnsing*lnsing.
compute isl=lnsing*lnplabor.
compute isu=lnsing*lnpsurf.
compute isi=lnsing*lnirate.
compute ism=lnsing*lnmiles.

compute ill=lnplabor*lnplabor.
compute ilu=lnplabor*lnpsurf.
compute ili=lnplabor*lnirate.
compute ilm=lnplabor*lnmiles.

compute iuu=lnpsurf*lnpsurf.
compute iui=lnpsurf*lnirate.
compute ium=lnpsurf*lnmiles.

compute iii=lnirate*lnirate.
compute iim=lnirate*lnmiles.

compute imm=lnmiles*lnmiles.

select if (auto > 0 and comb > 0 and sing > 0 and totexp > 0 ).
select if (plabor > 0 and psurf > 0).

regression variables=lnvarexp lnauto lncomb lnsing lnplabor lnpsurf lnirate
  /dependent=lnvarexp
  /method=enter
  /save resid (resid3).

compute wres3=1/(resid3*resid3)
...
regression variables=lnvarexp lnauto lncomb lnsing lnplabor lnpsurf lnirate
  iaa iac ias iau iai iam icc ics icu ici icm iss isu isi ism
  iuu iui ium iii iim imm
  /dependent=lnvarexp
  /method=enter
  /save resid (resid5).
compute wres5=1/(resid5*resid5).
regression variables=lnvarexp lnauto lncomb lnsing lnplabor lnpsurf lnirate
  iaa iac ias iau iai iam icc ics icu ici icm iss isu isi ism
  iuu iui ium iii iim imm
  /regwgt wres5
