...
*DATA LIST FILE 'C:\HIGHWAYS\HWY92C.TXT' FREE / STATE (A4) YEAR
...
*   PSTCON93 POP90 SQMILES POPDENS.
*
*means variables= PGSAL93 PGSAL94 PBCON93 PEXCAV93 PRESTL93 PSTSTL93
*   PSTCON93 POP90 SQMILES POPDENS by state.
*
*save outfile='C:\HIGHWAYS\HIGHWAY4.X'.

get file='C:\HIGHWAYS\HIGHWAY4.X'.
...
compute PMAINT=PBCON93.

compute pexcav=pexcav93/mexcav93.
compute psurf=pbcon93/mbcon93.
compute pstruc=prestl93/mrestl93+pststl93/mststl93+pstcon93/mstcon93.
...
compute iauau=lnauto*lnauto.
compute iauco=lnauto*lncomb.
compute iausi=lnauto*lnsing.
compute iaupu=lnauto*lnpurb.
compute iaumi=lnauto*lnmiles.
compute icoco=lncomb*lncomb.
compute icosi=lncomb*lnsing.
compute icopu=lncomb*lnpurb.
compute icomi=lncomb*lnmiles.
compute isisi=lnsing*lnsing.
compute isipu=lnsing*lnpurb.
compute isimi=lnsing*lnmiles.
compute ipupu=lnpurb*lnpurb.
compute ipumi=lnpurb*lnmiles.
compute imimi=lnmiles*lnmiles.

compute ipcpc=lnPCONST*lnPCONST.
compute ipcpl=lnPCONST*lnPLABOR.
compute ipcpd=lnpconst*lnpdens.
compute iplpl=lnplabor*lnplabor.
compute iplpd=lnplabor*lnpdens.
compute ipdpd=lnpdens*lnpdens.

compute iaupl=lnauto*lnplabor.
compute iaupd=lnauto*lnpdens.
compute iaupc=lnauto*lnpconst.

compute icopl=lncomb*lnplabor.
compute icopd=lncomb*lnpdens.
compute icopc=lncomb*lnpconst.

compute isipl=lnsing*lnplabor.
compute isipd=lnsing*lnpdens.
compute isipc=lnsing*lnpconst.

compute PKAP = capnew/miles.
compute lnPKAP=ln(PKAP).

compute ipkpk=lnpkap*lnpkap.
compute ipkpc=lnpkap*lnpconst.
compute ipkpl=lnpkap*lnplabor.
compute ipkpd=lnpkap*lnpdens.
compute ipkau=lnpkap*lnauto.
compute ipksi=lnpkap*lnsing.
compute ipkco=lnpkap*lncomb.
compute ipkmi=lnpkap*lnmiles.
...
* 1. LINEAR.
*
*regression variables= totexp auto comb sing PLABOR PDENS PCONST PKAP
*  /dependent=totexp
*  /method=enter
*  /save resid (resid3).
*
...
** 2. COBB - DOUGLAS.
*
*regression variables= lntotexp lnauto lncomb lnsing lnPLABOR lnPDENS lnPCONST
*  lnPKAP
*  /dependent=lntotexp
*  /method=enter
*  /save resid (resid4).
*
*compute wres2=1/(resid4*resid4).
*
*regression variables= lntotexp lnauto lncomb lnsing lnPLABOR lnPDENS lnPCONST
*  lnPKAP
*  /regwgt wres2
...
*
*regression variables= lntotexp lnauto lncomb lnsing lnPLABOR lnPDENS lnPCONST
*  lnPKAP ipkpk ipkpc ipkpl ipkpd ipkau ipksi ipkco
*  iauau iauco iausi iaupl iaupd iaupc icoco icosi icopl icopd icopc
*  isisi isipl isipd isipc ipcpc ipcpl ipcpd iplpl iplpd ipdpd
*  /CRITERIA TOLERANCE (0.0000001)
*  /dependent=lntotexp
*  /method=enter
*  /save resid (resid5).

*compute wres3=1/(resid5*resid5).
*
*regression variables= lntotexp lnauto lncomb lnsing lnPLABOR lnPDENS lnPCONST
*  lnPKAP ipkpk ipkpc ipkpl ipkpd ipkau ipksi ipkco
*  iauau iauco iausi iaupl iaupd iaupc icoco icosi icopl icopd icopc
...
means variables= PGSAL93 PGSAL94 PBCON93 PEXCAV93 PRESTL93 PSTSTL93
   PSTCON93 POPDENS TOTEXP AUTO COMB SING PKAP MILES by x.

means variables=PCONST by x.
